...
const jsonLocalization=JSON.parse(`
{
    "en": {
        ...
        "igBanNote": "If the page doesn't load anymore, your IP may have been temporarily limited/banned from anonymous Instagram browsing. If you still need to view the page, you can try using one of the external services listed below:"
	},
    "it": {
        ...
        "igBanNote": "Se la pagina non carica oltre, il tuo indirizzo IP potrebbe essere stato temporaneamente limitato/bandito dalla navigazione anonima su Instagram. Se hai comunque bisogno di vedere la pagina, puoi provare usando uno dei servizi esterni elencati qui sotto:"
    }
}
`);
(async function(){
...
function checkPageReady(){
  if((document.getElementsByTagName("BUTTON").length>0)||(document.getElementsByTagName("FORM").length>0)||(document.getElementsByClassName("signup_login_buttons").length>0)||(document.getElementsByTagName("shreddit-post").length>0)||(document.getElementById("layers")!==null)||(document.getElementById("mcont")!==null)||(document.getElementsByTagName("VIDEO").length>0)){//If buttons are there, the page should have loaded. At least that's what I see...
    ...
    if(document.location.href.includes("facebook.com")){
      ...
      igLoadOutsideNote_callback(document.querySelector("MAIN").firstChild);
    }
      ...
      var interval_0=setInterval(tiktok_waitReady,500);
      var interval_1=setInterval(tiktok_clickList,500);
    }
  }
  else{
...
function igLoadOutsideNote(){
  console.log("Adding external services suggestion");
  var loader=document.querySelector("[data-visualcompletion=loading-state]");
  const config = { attributes: false, childList: true, subtree: true };
  const callback = function(mutationsList, observer) {
    if(!document.getElementById("extSvcNote")){
      for(const mutation of mutationsList) {
        for(let newNode of mutation.addedNodes){
          let loader_tmp=newNode.querySelector("[data-visualcompletion=loading-state]");
          if(loader_tmp){
            loader=loader_tmp;
            igLoadOutsideNote_callback(loader);
          }
        }
      }
    }
  };
  const articleObserver = new MutationObserver(callback);
  if(document.getElementsByTagName("ARTICLE")){
  	articleObserver.observe(document.getElementsByTagName("ARTICLE")[0], config);
  }
  if(loader){
    igLoadOutsideNote_callback(loader);
  }
}
function igLoadOutsideNote_callback(loader){
  ...
  if(document.querySelector("[role=tablist]")){ //if one of the main profile pages
    ...
    link_1.href="https://www.pixwox.com/profile/"+username; 
    ...
    link_1.href="https://www.pixwox.com/"; 
  ...
  console.log(note);
  loader.parentNode.parentNode.appendChild(note);
  try{
    note.setAttribute("style","border: 4px solid #E79; border-radius: 1em; padding: 1em"); //style doesn't apply and I don't know why
  }
  catch(e){
    console.log(e)
  }
...
function tiktok_waitReady(){
  let lockStyle=document.getElementById("tux-screen-lock");
  if(lockStyle!==null){
...
function tiktok_clickList(){
  ...
  if(list.length>0){
    if(list[0].getElementsByTagName("LI").length>0){
      ...
      tiktok_clearInterval(interval_1);
    }
  }
}
